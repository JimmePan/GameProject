...
Surface::Surface(int z, int sHdl, unsigned uType)
{
	...
	unsigned Tp = uType;
	_handle = sHdl;
	_angle = VGet(-SPD, 0.0f, -SPD);	//获得向量（-0.01,0，-0.01）
	_vertex[0].pos = VGet(vPos[Tp][0].x, vPos[Tp][0].y, vPos[Tp][0].z + WIDZ * z);		//坐标  坐上
	_vertex[0].norm = VGet(1.0f, 0.0f, 0.0f);											//法线
	_vertex[0].dif = GetColorU8(255, 255, 255, 255);									//颜色
	_vertex[0].spc = GetColorU8(0, 0, 0, 0);											//
	_vertex[0].u = 0.0f;																//纹理坐标
	_vertex[0].v = 0.0f;
	_vertex[0].su = 0.0f;																//辅助纹理坐标
	_vertex[0].sv = 0.0f;
...
{
	MATRIX Matrix = MGetRotX(csDefRotaX); //构建X轴旋转矩阵
